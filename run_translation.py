...
"""Run translation fine-tuning for BigBird.."""
...
flags.DEFINE_string(
    "src_vocab_model_file", "./en.model",
    "SPM")

flags.DEFINE_string(
    "tgt_vocab_model_file", "./zh.model",
    "SPM")
flags.DEFINE_integer(
    "num_decoder_layers", 6,
    "Number of hidden decoder layers in the model (less than encoder since decoder uses full attn).")

def input_fn_builder(data_dir, src_vocab_model_file, tgt_vocab_model_file, max_encoder_length,
  ...
  def _tokenize_example(src, tgt):
    src_tokenizer = tft.SentencepieceTokenizer(
        model=tf.io.gfile.GFile(src_vocab_model_file, "rb").read())
    if substitute_newline:
      src = tf.strings.regex_replace(src, "\n", substitute_newline)
    # Remove space before special tokens.
    src = tf.strings.regex_replace(src, r" ([<\[]\S+[>\]])", b"\\1")
    src_ids = src_tokenizer.tokenize(src)
    if isinstance(src_ids, tf.RaggedTensor):
      src_ids = src_ids.to_tensor(0)
    src_ids = src_ids[:max_encoder_length]

    tgt_tokenizer = tft.SentencepieceTokenizer(
        model=tf.io.gfile.GFile(tgt_vocab_model_file, "rb").read())
    # Remove newline optionally
    if substitute_newline:
      tgt = tf.strings.regex_replace(tgt, "\n", substitute_newline)
    # Remove space before special tokens.
    tgt = tf.strings.regex_replace(tgt, r" ([<\[]\S+[>\]])", b"\\1")
    tgt_ids = tgt_tokenizer.tokenize(tgt)
    # Add [EOS] (1) special tokens.
    suffix = tf.constant([1])
    tgt_ids = tf.concat([tgt_ids, suffix], axis=0)
    if isinstance(tgt_ids, tf.RaggedTensor):
      tgt_ids = tgt_ids.to_tensor(0)
    tgt_ids = tgt_ids[:max_decoder_length]
...
def model_fn_builder(transformer_config):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    total_loss = padded_cross_entropy_loss(
        ...
        transformer_config["tgt_vocab_size"])
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      src_tokenizer = tft.SentencepieceTokenizer(
          model=tf.io.gfile.GFile(transformer_config["src_vocab_model_file"],
                                  "rb").read())
      tgt_tokenizer = tft.SentencepieceTokenizer(
          model=tf.io.gfile.GFile(transformer_config["tgt_vocab_model_file"],
                                  "rb").read())
    ...
    #     return r1/len(label_sent), r2/len(label_sent), rl/len(label_sent)
      def bleu_py_func(label_sent, pred_sent):  
        metric = evaluate.load("sacrebleu")
        return metric.compute(predictions = pred_sent, references = [[label] for label in label_sent])['bleu']
      ...
      def metric_fn(loss, log_probs, label_ids, pred_ids):
        ...
        if not transformer_config["use_tpu"]:
          ...
          label_sent = src_tokenizer.detokenize(label_ids)
          label_sent = tf.strings.regex_replace(label_sent, r"([<\[]\S+[>\]])",
                                                b" \\1")
          pred_sent = tgt_tokenizer.detokenize(pred_ids)
          ...
          bleu_value = tf.compat.v1.py_func(
              func=bleu_py_func,
